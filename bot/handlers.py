...
async def extract_text_from_file(document) -> tuple[str, str | None]:
    ...
    try:
        ...
        return (
            "",
            "Ошибка при чтении файла. Попробуйте другой файл или скопируйте текст вручную.",
        )
...
async def process_text(user_id: int, text: str, reply_func) -> None:
    ...
    if not text.strip():
        await reply_func(
            "Из файла не удалось извлечь текст. Попробуйте другой файл или отправьте текст вручную."
        )
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text or not text.strip():
        await message.reply_text(
            "Пожалуйста, отправьте текст договора или задайте вопрос."
        )
        return
    ...
    if is_question:
        ...
        if fragments:
            ...
            system_prompt = SYSTEM_PROMPT_QUESTION.format(
                context=context_str, question=text
            )
            messages = [{"role": "system", "content": system_prompt}]
        else:
            await message.reply_text(
                "В базе знаний не найдено информации по вашему запросу. Пожалуйста, уточните вопрос или обратитесь к юристу."
            )
            return
    else:
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await message.reply_text(
        f"Текст из файла извлечён (длина: {len(text)} символов). Анализирую..."
    )
